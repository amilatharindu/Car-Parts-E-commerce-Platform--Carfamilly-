...
if (isset($_POST['place_order'])) {
    ...
    if (isset($_GET['get_id'])) {
        ...
        if ($get_product->rowCount() > 0) {
            while ($fetch_p = $get_product->fetch(PDO::FETCH_ASSOC)) {
                ...
                // Insert the order with the grand total in the price field
    ...
    // "Proceed to checkout" option (cart)
    } else {
        ...
        if ($verify_cart->rowCount() > 0) {
            ...
            while ($f_cart = $verify_cart->fetch(PDO::FETCH_ASSOC)) {
                ...
                if ($fetch_p) {
                    ...
                    $insert_order = $conn->prepare("INSERT INTO `orders`(id, user_id, name, number, email, address, address_type, method, product_id, price, qty) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
                    $insert_order->execute([create_unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $f_cart['product_id'], $sub_total, $f_cart['qty']]);
                }
            }

            // After inserting all items, insert a summary order for the grand total
            $summary_order = $conn->prepare("INSERT INTO `orders`(id, user_id, name, number, email, address, address_type, method, price) VALUES(?,?,?,?,?,?,?,?,?)");
            $summary_order->execute([create_unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $grand_total]);
